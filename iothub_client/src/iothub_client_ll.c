...
IOTHUB_CLIENT_RESULT IoTHubClient_LL_SetOption(IOTHUB_CLIENT_LL_HANDLE iotHubClientHandle, const char* optionName, const void* value)
{
    ...
    else
    {
...
#else
            LogError("TOPTION_BLOB_TRANSFER_TIMEOUT option being set with DONT_USE_UPLOADTOBLOB compiler switch");
            result = IOTHUB_CLIENT_ERROR;
        ...
        else
        {
            ...
            {
                /*Codes_SRS_IOTHUBCLIENT_LL_30_013: [ If the DONT_USE_UPLOADTOBLOB compiler switch is undefined, IoTHubClient_LL_SetOption shall pass unhandled options to IoTHubClient_UploadToBlob_SetOption and return its result. ]*/
                result = IoTHubClient_LL_UploadToBlob_SetOption(handleData->uploadToBlobHandle, optionName, value);
                if(result != IOTHUB_CLIENT_OK)
                {
                    LogError("unable to IoTHubClient_LL_UploadToBlob_SetOption");
